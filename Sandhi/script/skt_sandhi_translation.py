# -*- coding: utf-8 -*-
"""skt_sandhi_translation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1SlzrrxFmGI0LLz7Td9NFvOgQut0GJ5dg
...
for samh_examples, pada_examples in train_examples.batch(3).take(1):
  for samh in samh_examples.numpy():
    print(samh.decode('utf-8'))

  print()

  for pada in pada_examples.numpy():
    print(pada.decode('utf-8'))

# train_pada = train_examples.map(lambda samh, pada: pada)
# train_samh = train_examples.map(lambda samh, pada: samh)
...
tokenizers = tf.saved_model.load(work_dir + model_name)

[item for item in dir(tokenizers.pada) if not item.startswith('_')]

# for pada in pada_examples.numpy():
#   print(pada.decode('utf-8'))

encoded = tokenizers.pada.tokenize(pada_examples)

# for row in encoded.to_list():
#   print(row)

round_trip = tokenizers.pada.detokenize(encoded)
# for line in round_trip.numpy():
#   print(line.decode('utf-8'))

tokens = tokenizers.pada.lookup(encoded)
# tokens

def tokenize_pairs(samh, pada):
    samh = tokenizers.samh.tokenize(samh)
    # Convert from ragged to dense, padding with zeros.
    samh = samh.to_tensor()

    pada = tokenizers.pada.tokenize(pada)
    # Convert from ragged to dense, padding with zeros.
    pada = pada.to_tensor()
    return samh, pada
...
input_vocab_size = tokenizers.samh.get_vocab_size().numpy()
target_vocab_size = tokenizers.pada.get_vocab_size().numpy()
...
checkpoint_path = work_dir + "/checkpoints/train"
...
for epoch in range(EPOCHS):
  ...
  # inp -> samhitapatha, tar -> padapatha
...
class Translator(tf.Module):
  ...
  def __call__(self, sentence, max_length=20):
    ...
    sentence = self.tokenizers.samh.tokenize(sentence).to_tensor()

    encoder_input = sentence

    # as the target is english, the first token to the transformer should be the
    # english start token.
    start_end = self.tokenizers.pada.tokenize([''])[0]
    ...
    text = tokenizers.pada.detokenize(output)[0]  # shape: ()

    tokens = tokenizers.pada.lookup(output)[0]
...
sentence = "yfo hatvfAhim fariNAt saptfa sfinDUn"
ground_truth = "yfaH hatvfA fahim fariNAt saptfa sfinDUn"
...
in_tokens = tokenizers.samh.tokenize(in_tokens).to_tensor()
in_tokens = tokenizers.samh.lookup(in_tokens)[0]
in_tokens
